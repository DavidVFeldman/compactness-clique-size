...
\begin{remark} It is not hard to find non-$T_1$ lp-compact spaces having infinite cliques (consider, e.g., $\N \times \{0,1\}$ with $\N$ discrete and $\{0,1\}$ indiscrete, and 
...
\begin{corollary}\label{starcompact} A space with finite cliques is strongly star-compact{\blue .}
\end{corollary} 
...
\bibitem{Bern} A. R. Bernstein, A new kind of compactness for topological spaces, {\blue Fund.\ Math.}\ {\bf 66} (1970), 185--193.

\bibitem{Cao} J.~Cao, Recent progress in some aspects of star covering properties, RIMS K\^oky\^uroku {\bf 1248} (2002), 12--17.


\bibitem{HL} A.~F.~Holmsen and S.~Lee, Orthogonal colorings of the sphere, Mathematika {\bf 62} (2016), 
{\blue 492--501}
...
%{\bf Verification}{[Old --- to be updated]} 
\tempout{The following have been formally verified in Lean 4 against Mathlib (toolchain \texttt{v4.28.0}), with no axioms beyond Lean's standard three: Lemmas \ref{compactbounded} and \ref{finitecc}, Proposition \ref{galois} with Corollary \ref{covers}, Proposition \ref{preserve}, the present theorem, clauses (i) and (iii) of Corollary \ref{suffconds}, the boundedness assertion of Remark \ref{cofinite}, the clique bound of Remark \ref{sphere}, and the two combinatorial lemmas underlying Remark \ref{betaomega} (splitting of bounded transversals inside an ultrafilter --- in a canonical, choice-free form via cell minima --- and the infinitude of selection-complement intersections). Hypotheses are formalized in the slightly stronger primitive forms actually consumed by the proofs; e.g., the present theorem is verified from ``every sequence in $X^{\omega}$ has a cluster point,'' which countable compactness of $X^{\omega}$ implies. Not formalized (out of commissioned scope): clause (ii) of Corollary \ref{suffconds}, Remark \ref{omegaone}, Corollary \ref{metacompact}, and the topological (as opposed to combinatorial) content of Remark \ref{betaomega}. Repository link to be added.}
